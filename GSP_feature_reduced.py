...
def train_model():
    ...
    state = Input(shape=(180, 11))
    goal = Input(shape=(180, 3))
    ...
    lstm_1 = LSTM(100, return_sequences=True, return_state=False, stateful=False)(concat_2)
...
def test_model():
    ...
    lstm_1 = LSTM(100, return_sequences=True, return_state=False, stateful=True)(concat_2)
...
def train(model):
    ...
    # state_feed_1 = data[:, :, 0:8]
    # state_feed_2 = data[:, :, 14:17]
    # state_feed = np.append(state_feed_1, state_feed_2, axis=-1)
...
def test(model_for_25_nets):
    ...
    model_for_25_nets.load_weights('ppp-reduced-category.993-0.7849.hdf5', by_name=True)
