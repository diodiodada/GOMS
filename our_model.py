...
def train_our_model(model):
    ...
    data = get_data('data/PP-1-paths-1000-[0, 1, 2, 3].p', usage="")
    ...
    tf_board = TensorBoard(log_dir='./logs-i',
                           histogram_freq=0,
                           write_graph=True,
                           write_images=False,
                           embeddings_freq=0,
    ...
    model_checkpoint = ModelCheckpoint('weights-i/I.{epoch:d}-{val_loss:.8f}.hdf5',
                                       monitor='val_loss',  # here 'val_loss' and 'loss' are the same
                                       verbose=1,
                                       save_best_only=True,
                                       save_weights_only=True)
...
def test_our_model(model):
    ...
    model.load_weights('weights-i/1-I.999-0.27311218.hdf5', by_name=True)
